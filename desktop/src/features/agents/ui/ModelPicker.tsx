...
export function ModelPicker({
  ...
  const handleModelChange = async (modelId: string) => {
    ...
    try {
      if (isLiveSwitch) {
        ...
        if (outcome === "failed") {
          toast.error(t("agents.modelSwitchFailedKept"));
          return;
        }
        if (outcome === "not_delivered") {
          ...
          toast.error(t("agents.modelSwitchNotDelivered"));
          return;
        }
        if (outcome === "pending") {
          ...
          toast.info(t("agents.modelSwitchPending"));
          onModelChanged?.();
          return;
        }
  ...
  return (
    <span className="inline-flex items-center gap-1.5">
      ...
      {needsRestart ? (
        <span className="text-2xs text-warning">{t("agents.restartToApply")}</span>
      ) : null}
    </span>
  );
}
